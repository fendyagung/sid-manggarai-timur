...
echo "=== GUARANTEED NUCLEAR SYNC ===\n";

// Phase 1: Pure Binary Cleaning
echo "Cleaning all records of invisible garbage...\n";
foreach (Desa::all() as $d) {
    // Keep only clean printable characters
...
foreach ($masterData as $kecamatan => $data) {
    ...
    foreach ($list as $vName) {
        ...
        // Find ALL records matching this precise clean name globally
        ...
        foreach ($matches as $other) {
            ...
            echo "  Merging {$other->nama_desa} (ID {$other->id} -> {$target->id})\n";
            ...
            if (Schema::hasColumn('dokumens', 'receiver_desa_id')) {
