        ...
        <div class="bg-white rounded-3xl shadow-xl border border-slate-100 overflow-hidden">
            ...
            <div class="divide-y divide-slate-50">
                @forelse($inbox as $doc)
                        <div
                            class="p-6 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-all flex flex-wrap sm:flex-nowrap items-center justify-between gap-4 min-h-[100px] overflow-visible">
                            <div class="flex-1 min-w-0 overflow-visible">
                                ...
                                <h3
                                    class="font-bold text-slate-800 dark:text-slate-100 text-lg leading-snug transition-colors break-all line-clamp-2">
                                    {{ $doc->judul }}
                                </h3>
                            ...
                            {{-- Only download for Inbox (Recipients) --}}
                            <a href="{{ route('dashboard.dokumen.download', $doc->id) }}"
                                class="flex items-center gap-2 px-4 py-3 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-xl shadow-lg shadow-emerald-500/20 transition-all transform hover:-translate-y-1"
                                title="Download">
    ...
    <div class="bg-white rounded-3xl shadow-xl border border-slate-100 overflow-hidden">
        ...
        <div class="divide-y divide-slate-50">
            @forelse($outbox as $doc)
                <div
                    class="p-6 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-all flex flex-wrap sm:flex-nowrap items-center justify-between gap-4 min-h-[100px] overflow-visible">
                    <div class="flex-1 min-w-0 overflow-visible opacity-80">
                        <div class="flex items-center gap-2 mb-1">
                        ...
                        <h3
                            class="font-bold text-slate-800 dark:text-slate-100 leading-tight transition-colors break-words line-clamp-2">
                            {{ $doc->judul }}
                        </h3>
                    ...
                    <div class="flex flex-row items-center gap-4 shrink-0 min-w-fit">
                        {{-- Only Delete/Cancel for Outbox (Senders) --}}
                        <form action="{{ route('dashboard.dokumen.destroy', $doc->id) }}" method="POST" class="inline-block"
                            onsubmit="return confirm('Batalkan pengiriman berkas ini?')">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
                                class="flex items-center gap-2 px-4 py-3 bg-red-50 hover:bg-red-100 text-red-500 text-xs font-bold rounded-xl transition-all"
                                title="Hapus">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24"
                                    stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
